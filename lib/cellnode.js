...
let {findCellTag, reTag, reBlock, reVariableNoSpace, reXv, imgSplit, blockSplit } = require('./celltag');
...
class Segment extends XNode {
    ...
    get nodeTag() {
        let text = this.text.replace(RegExp('{% *xv'), '{{').replace('%}', '}}');
...
function getCellNode(cellModel, row, col) {
    ...
    if(cellModel && (cellModel.type === Cell.Types.String ||
        ...
        if(reTag.test(text)) {
            ...
            if(text==="") {
                cellModel.value = "";
            } else if(reVariableNoSpace.test(text)){
                cellNode = new XvCell(cellModel, row, col, text, false);
            } else if(reXv.test(text)) {
